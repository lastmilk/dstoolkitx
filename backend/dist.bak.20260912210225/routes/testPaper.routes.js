...
router.post('/generate', asyncHandler(async (req, res) => {
    ...
    const creditResult = await tryConsumeCredits(req.user.id, 'TEST_PAPER', cost, `生成试卷：${requirement.slice(0, 30)}`);
    if (!creditResult.ok) {
    ...
    const placeholder = await prisma.testPaper.create({
        data: {
            ...
            status: 'GENERATING',
            scope: { conversationIds, keywords },
        },
        select: { id: true },
    });
    // 提交异步任务
...
//# sourceMappingURL=testPaper.routes.js.map