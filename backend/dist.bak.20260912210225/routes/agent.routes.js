...
router.post('/:id/message', asyncHandler(async (req, res) => {
    ...
    if (!conv)
        return res.status(404).json({ error: '对话不存在' });
    // 额度限制：消耗 AI 积分
    const creditResult = await tryConsumeCredits(req.user.id, 'AGENT_CHAT', env.agentChatCreditCost, `Agent 续聊对话 #${id}`);
    if (!creditResult.ok) {
    ...
    try {
        ...
        res.write(`data: ${JSON.stringify({ done: true, model: result.model, toolCalls, balance: creditResult.balance })}\n\n`);
    }
    catch (e) {
