...
function findEntryName(zip, filename) {
 ...
 */
function getNodeFragType(mapping, nodeId) {
    const node = mapping[nodeId];
    if (!node || !node.message)
        return null;
    const frags = node.message.fragments || [];
...
function extractTurns(mapping) {
    const rootNode = Object.values(mapping).find((n) => n && n.parent === null);
    if (!rootNode)
        return [];
    const getChildren = (id) => {
        const node = mapping[id];
        return node && Array.isArray(node.children) ? node.children.map((c) => String(c)) : [];
    };
    const isUser = (id) => getNodeFragType(mapping, id) === 'REQUEST';
    const isAssistant = (id) => getNodeFragType(mapping, id) === 'RESPONSE';
    // 1. 计算主链（最后一个子节点为延续）+ 主链顺序
    const mainChain = new Set();
    const mainChainOrder = [];
    const visited = new Set();
    let current = String(rootNode.id);
    ...
    for (const nodeId of mainChainOrder) {
        ...
        for (const childId of getChildren(nodeId)) {
            ...
            for (const subChildId of getChildren(childId)) {
                if (!isUser(subChildId))
                    continue;
                if (mainChain.has(subChildId))
                    continue; // 主链上的 USER = 下一 Turn，跳过
                // SubTurn 的 ASSISTANT 回复 = 该 USER 节点的首个 ASSISTANT 子节点
                let subAssistantId = null;
                ...
                if (version.subTurns.length >= 6)
                    break;
            }
            turn.versions.push(version);
            if (turn.versions.length >= 6)
                break;
        }
        turns.push(turn);
    }
 ...
 */
function buildNodeIndexMap(turns) {
    const map = new Map();
 ...
 */
export function processConversation(c) {
    ...
    const visit = (nodeId) => {
        ...
        if (msg) {
            ...
            const ts = msg.inserted_at ? new Date(msg.inserted_at) : new Date();
...
export function buildDeepseekUser(userJson) {
 ...
 */
export async function streamConversationsBuffer(conversationsBuffer, onConversation, onProgress) {
    const stream = streamArray.withParserAsStream();
    let loaded = 0;
    try {
        await pipeline(Readable.from(conversationsBuffer), stream, async function (source) {
        ...
        const arr = JSON.parse(conversationsBuffer.toString('utf8'));
        for (const c of arr) {
 ...
 */
export async function parseDeepseekZipStream(buffer, onConversation, onProgress) {
 ...
 */
export async function parseDeepseekZip(buffer) {
    const conversations = [];
...
//# sourceMappingURL=deepseekParser.js.map