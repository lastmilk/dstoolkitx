...
function keyPublic(t: {
  ...
  return {
    ...
    prefix: t.prefix,
    masked: `${t.prefix}****`,
    scope: t.containerId == null ? ('global' as const) : ('container' as const),
...
router.post('/username', asyncHandler(async (req: AuthedRequest, res) => {
  ...
  if (clash) return res.status(409).json({ error: '该 Git 用户名已被占用' })
