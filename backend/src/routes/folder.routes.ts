...
router.post('/', asyncHandler(async (req: AuthedRequest, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: '文件夹名称必填' })
...
router.post('/:id/assign', asyncHandler(async (req: AuthedRequest, res) => {
  const folderId = Number(req.params.id)
  const parsed = assignSchema.safeParse(req.body)
  if (!parsed.success) return res.status(400).json({ error: 'conversationId 必填' })
...
router.delete('/:id/assign/:convId', asyncHandler(async (req: AuthedRequest, res) => {
  const folderId = Number(req.params.id)
...
router.post('/tags', asyncHandler(async (req: AuthedRequest, res) => {
  const parsed = tagSchema.safeParse(req.body)
  if (!parsed.success) return res.status(400).json({ error: '标签名称必填' })
...
router.post('/tags/:id/assign', asyncHandler(async (req: AuthedRequest, res) => {
...
router.delete('/tags/:id', asyncHandler(async (req: AuthedRequest, res) => {
  const tagId = Number(req.params.id)
