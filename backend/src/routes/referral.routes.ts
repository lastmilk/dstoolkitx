...
router.get('/info', asyncHandler(async (req: AuthedRequest, res) => {
...
router.post('/links', asyncHandler(async (req: AuthedRequest, res) => {
  const code = genCode()
...
router.post('/bind', asyncHandler(async (req: AuthedRequest, res) => {
  ...
  if (!referrer) return res.status(404).json({ error: '邀请码无效' })
  if (referrer.id === req.user!.id) return res.status(400).json({ error: '不能邀请自己' })
  ...
  if (me.referredById) return res.json({ ok: true, message: '已绑定过邀请人' })
...
function env_frontendOrigin(): string {
