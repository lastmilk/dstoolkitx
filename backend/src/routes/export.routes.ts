...
import { convert, EXPORT_FORMATS, type ExportFormat, type ExportConversation } from '../services/exportConverter.js'
...
function parseConvIds(req: AuthedRequest): number[] | undefined {
  if (Array.isArray(req.query.conversationIds)) {
    return (req.query.conversationIds as string[]).map(Number)
  }
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    const { configId, format, limit } = req.body as {
      configId?: number
      format?: ExportFormat
      limit?: number
    }
    if (!configId) return res.status(400).json({ error: '缺少 configId' })
    if (!format) return res.status(400).json({ error: '缺少 format' })
...
router.get(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    if (convs.length === 0) return res.status(400).json({ error: '没有可导出的对话' })
