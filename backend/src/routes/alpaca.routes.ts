...
router.post('/convert', asyncHandler(async (req: AuthedRequest, res) => {
  ...
  const conversationIds = Array.isArray(req.query.conversationIds)
    ? (req.query.conversationIds as string[]).map(Number)
  ...
  const data = multiTurn ? toAlpacaMulti(convs as any) : toAlpacaSingle(convs as any)
  return res.json({ count: data.length, data })
}))

// GET /api/alpaca/download?configId=&conversationIds=&multiTurn=true
router.get('/download', asyncHandler(async (req: AuthedRequest, res) => {
  const configId = Number(req.query.configId)
  if (!configId) return res.status(400).json({ error: '缺少 configId' })
  const multiTurn = String(req.query.multiTurn).toLowerCase() === 'true'
  const conversationIds = Array.isArray(req.query.conversationIds)
