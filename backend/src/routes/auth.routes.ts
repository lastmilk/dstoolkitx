...
import { verifyJwt, verifyAnyToken, type AuthedRequest } from '../middleware/auth.js'
...
function smsErrorStatus(e: unknown): number {
  return (e as { status?: number })?.status === 503 ? 503 : 400
}
...
async function findOrCreatePhoneUser(phone: string) {
  ...
  if (!username) throw Object.assign(new Error('用户名生成失败，请重试'), { status: 500 })
...
router.post('/sms/send', requireCaptcha, asyncHandler(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: '手机号格式错误' })
  const { phone, countryCode = '86' } = parsed.data
  if (!hit(`sms:${phone}`, 60_000, 1)) return res.status(429).json({ error: '发送太频繁，请 1 分钟后再试' })
  if (!hit(`smsday:${phone}`, 86_400_000, 10)) return res.status(429).json({ error: '今日验证码发送次数已达上限' })
  if (!hit(`smsip:${req.ip || 'unknown'}`, 3_600_000, 30)) return res.status(429).json({ error: '当前网络发送次数过多，请稍后再试' })
  try {
...
router.post('/sms/login', asyncHandler(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: '手机号或验证码格式错误' })
...
router.post('/phone/send-code', verifyJwt, requireCaptcha, asyncHandler(async (req: AuthedRequest, res) => {
  const parsed = phoneSchema.safeParse(req.body)
  if (!parsed.success) return res.status(400).json({ error: '手机号格式错误' })
  const { phone, countryCode = '86' } = parsed.data
...
router.post('/phone/bind', verifyAnyToken, asyncHandler(async (req: AuthedRequest, res) => {
  const parsed = z.object({
    phone: z.string().regex(/^\d{5,20}$/),
    code: z.string().min(4).max(8),
    countryCode: z.string().regex(/^\d{1,4}$/).optional(),
