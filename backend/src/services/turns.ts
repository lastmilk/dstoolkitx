import type { Turn, Version, SubTurn } from './deepseekParser.js'
 ...
 */
export function aggregateTurnsFromMessages(messages: FlatMessage[]): Turn[] {
  const turnMap = new Map<
    number,
    {
      ...
      versions: Map<
        number,
        { assistantNodeId: string; subTurns: Map<number, { userNodeId: string; assistantNodeId: string | null }> }
      >
    }
  >()
  ...
  for (const m of messages) {
    ...
    if (m.versionIndex === null) {
      // Turn 用户提问节点
      if (m.role === 'USER') turn.userNodeId = m.nodeId
      continue
    }
    ...
    if (m.subTurnIndex === null) {
      ...
      if (m.role === 'ASSISTANT') ver.assistantNodeId = m.nodeId
      continue
    }
    ...
    if (m.role === 'USER') sub.userNodeId = m.nodeId
    else if (m.role === 'ASSISTANT') sub.assistantNodeId = m.nodeId
  }
