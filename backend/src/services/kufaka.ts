...
import type { CardDuration } from '@prisma/client'

export interface KufakaValidation {
  valid: boolean
  tier?: Tier
 ...
 */
export async function validateKufakaCard(code: string): Promise<KufakaValidation | null> {
  if (!env.kufakaEnabled) return null
  try {
    ...
    const data = (await res.json()) as KufakaValidation
    return data
  } catch (e) {
    console.warn('[kufaka] 校验失败，降级为本地卡密校验:', (e as Error)?.message)
    return null
  }
}

/** 将 kufaka 返回的等级字符串映射为内部 Tier；未知则返回 null */
export function parseKufakaTier(s: string | undefined): Tier | null {
  if (!s) return null
  const up = String(s).toUpperCase()
  if (up === 'PRO' || up === 'PLUS' || up === 'ULTIMATE') return up
  return null
}
