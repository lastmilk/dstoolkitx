 ...
 */
export async function upsertConversations(userId: number, configId: number, convs: ParsedConversation[]) {
 ...
 */
export function repoConversationToParsed(json: any): ParsedConversation | null {
  ...
  const messages = rawMessages
    .filter((m: any) => m && typeof m.content === 'string' && m.nodeId != null)
    .map((m: any) => ({
      nodeId: String(m.nodeId),
      parentId: m.parentId != null ? String(m.parentId) : null,
      role: m.role === 'ASSISTANT' || m.role === 'RESPONSE' ? ('ASSISTANT' as const) : ('USER' as const),
  ...
  return {
    ...
    insertedAt,
    updatedAt,
    mapping: (json.mapping ?? {}) as Record<string, unknown>,
    messages,
