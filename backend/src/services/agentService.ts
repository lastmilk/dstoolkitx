...
import { callStepfun, type ChatMessage } from './aiClient.js'
import type { Skill, McpServer } from '@prisma/client'
 ...
 */
export async function loadUserSkills(userId: number): Promise<string> {
 ...
 */
export async function loadUserMcpTools(userId: number): Promise<AgentTool[]> {
  ...
  if (server.transport === 'HTTP' && server.url) {
    try {
      ...
      if (server.headers && typeof server.headers === 'object') {
        Object.assign(headers, server.headers as Record<string, string>)
      }
...
export async function runAgent(opts: AgentRunOptions): Promise<AgentRunResult> {
  ...
  const mcpServerMap = new Map<number, McpServer>()
  for (const t of tools) {
    if (t.mcpServerId) {
      const server = await prisma.mcpServer.findUnique({ where: { id: t.mcpServerId } })
      if (server) mcpServerMap.set(t.mcpServerId, server)
    }
  }
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    for (const tc of result.toolCalls) {
      ...
      if (toolName.startsWith(mcpPrefix)) {
        ...
        const server = [...mcpServerMap.values()].find(
          (s) => sanitizeName(s.name) === serverName,
        )
        if (server) {
