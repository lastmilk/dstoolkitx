 ...
 */
export function parseRepoName(name: string): GitRepoRef | null {
  const m = /^u(\d+)_c(\d+)\.git$/.exec(name)
  if (!m) return null
  const userId = Number(m[1])
  const containerId = Number(m[2])
  if (!Number.isInteger(userId) || !Number.isInteger(containerId)) return null
 ...
 */
async function authBySessionCredential(key: string): Promise<GitAuthResult | null> {
  try {
    if (key.startsWith(TOKEN_PREFIX)) {
      // dstk_ API 令牌（APP 端登录态，与 /v1/* verifyApiToken 同源）
      if (!isApiTokenFormat(key)) return null
...
export async function gitBasicAuth(
  req: Request,
  repo: GitRepoRef,
): Promise<{ ok: true; auth: GitAuthResult } | { ok: false; status: number; message: string }> {
  ...
  if (idx < 0) return { ok: false, status: 401, message: 'Basic 认证头格式错误' }
