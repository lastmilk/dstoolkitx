...
function isZero(sha: string) {
  return /^0+$/.test(sha)
}

/** 解析 "u<uid>_c<cid>.git" */
function parseRepo(name: string): { userId: number; containerId: number } | null {
...
async function processQueueFile(file: string): Promise<boolean> {
    ...
    for (const ch of changes) {
      if (ch.path === 'container.json') {
        ...
        try {
          ...
          const data: any = {}
          if (typeof meta.name === 'string' && meta.name && meta.name !== container.name) data.name = meta.name
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        if (ok) fs.rmSync(path.join(QUEUE_DIR, file), { force: true })
      } catch (e: any) {
...
/** 推送完成后即时触发一轮镜像 */
export function kickMirror(_repoRef: { userId: number; containerId: number }) {
