...
export function toCsv(convs: ExportConversation[]): string {
...
export function toMarkdown(convs: ExportConversation[]): string {
  const parts: string[] = []
  parts.push(`# 对话记录导出\n`)
...
export function toHtml(convs: ExportConversation[]): string {
...
export function toAlpacaSingle(convs: ExportConversation[]): any[] {
  const out: any[] = []
  for (const c of convs) {
...
export function toAlpacaMulti(convs: ExportConversation[]): any[] {
  return convs.map((c) => ({
...
export function toJson(convs: ExportConversation[]): any {
  return convs.map((c) => ({
...
export function convert(
  format: ExportFormat,
  convs: ExportConversation[],
): { content: string; contentType: string; fileExt: string } {
  switch (format) {
    case 'json':
      return {
...
export const EXPORT_FORMATS: Array<{ id: ExportFormat; name: string; desc: string; ext: string }> = [
