...
import { callDeepseek, type ChatMessage } from './aiClient.js'
...
${knowledgeText}`
  ...
  try {
    ...
    if (!match) throw new Error('AI 返回格式异常，无法解析试卷')
  ...
  for (const q of rawQuestions) {
    ...
    if (!type || !TYPE_LABELS[type]) continue
 ...
 * 将生成的试卷持久化到数据库。
 */
export async function saveTestPaper(
  userId: number,
  paper: GeneratedTestPaper,
  scope?: { conversationIds?: number[]; keywords?: string[] },
): Promise<number> {
