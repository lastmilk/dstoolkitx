...
import { TIER_RANK, TIER_LIMITS, getUserUsage } from '../utils/quota.js';
/** 判定用户当前有效等级（考虑年费到期）。永久等级永不过期。 */
export function resolveEffectiveTier(user) {
    if (user.isPermanentTier)
        return user.tier;
    if (user.tier === 'FREE')
        return 'FREE';
...
/** 获取用户会员状态概览（等级 + 用量 + 配额） */
export async function getTierStatus(userId) {
 ...
 */
export async function activateTierWithCard(userId, card) {
    const user = await prisma.user.findUniqueOrThrow({
        where: { id: userId },
