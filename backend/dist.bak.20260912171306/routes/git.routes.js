...
async function ensureRepo(repoFsName) {
...
function pipeCgiToRes(cgiStdout, res) {
    let buf = Buffer.alloc(0);
    let headersDone = false;
    cgiStdout.on('data', (chunk) => {
        if (headersDone) {
            res.write(chunk);
            return;
        }
        ...
        for (const line of headerText.split('\r\n')) {
            ...
            if (k === 'status')
                status = Number.parseInt(v, 10) || 200;
            else
                res.setHeader(k, v);
        }
        res.status(status);
    ...
    cgiStdout.on('end', () => {
        if (!headersDone)
            res.status(502).end('git http-backend: empty response');
        else
            res.end();
    });
}
router.all('/:repo/*', async (req, res) => {
    ...
    if (!repoRef)
        return res.status(404).json({ error: '仓库地址不合法：应为 /git/u<userId>_c<containerId>.git/*' });
    ...
    const isReceive = req.method === 'POST' && subPath === 'git-receive-pack';
