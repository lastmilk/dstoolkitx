...
import { parseDeepseekZip, streamConversationsBuffer, extractZipEntries, buildDeepseekUser, } from '../services/deepseekParser.js';
 ...
 */
async function streamUploadToCloud(userId, configId, conversationsBuffer) {
    let batch = [];
    let count = 0;
    const flush = async () => {
        if (batch.length === 0)
...
router.post('/', upload.single('file'), asyncHandler(async (req, res) => {
    ...
    if (!name)
        return res.status(400).json({ error: '请填写配置名称' });
...
router.put('/:id/upload', upload.single('file'), asyncHandler(async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: '请上传 zip 压缩包' });
...
router.get('/:id/conversations-batch', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    const config = await prisma.deepseekConfig.findFirst({ where: { id, userId: req.user.id } });
    if (!config)
        return res.status(404).json({ error: '配置不存在' });
...
router.get('/:id/conversations-lite', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    const config = await prisma.deepseekConfig.findFirst({ where: { id, userId: req.user.id } });
    if (!config)
        return res.status(404).json({ error: '配置不存在' });
...
router.get('/:id/conversations/:convId', asyncHandler(async (req, res) => {
...
router.get('/:id/git-info', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    const config = await prisma.deepseekConfig.findFirst({ where: { id, userId: req.user.id } });
    if (!config)
        return res.status(404).json({ error: '配置不存在' });
